...
class RegisterController extends Controller
{
    public function index(){
        ...
        if(auth()->user()->Tipo_Usuario==2 || auth()->user()->Tipo_Usuario==3){
            Log::warning('Usuario no autorizado intenta entrar a registrar usuario, usuario: '.auth()->user()->Usuario);
            return redirect()->back();
        }
        Log::info("Se ingresa a registrar un usuario, el administrador: ".auth()->user()->Usuario);
    ...
    public function register(Request $request){
        ...
        $user->save();
        Log::info("Se ha registrado un usuario por el administrador: ".auth()->user()->Usuario);
